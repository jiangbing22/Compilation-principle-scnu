...
// 打印自动机
void LALR::printAutomaton(LR1NODE* start) const {
    if (!start) return;
    ...
    while (!stateQueue.empty()) {
        LR1NODE* currentNode = stateQueue.front(); // 取出队列中的节点
        stateQueue.pop(); // 将节点从队列中移除

        // 打印当前状态编号
        std::cout << "State " << currentNode->stateNum << ":\n";
        ...
        for (const auto& item : currentNode->items) {
            ...
            std::cout << ", lookahead: " << item.lookahead << "\n";
        }

        // 打印当前状态的所有转移
        for (const auto& transition : currentNode->state) {
            std::cout << "  " << transition.first << " -> State "
                << transition.second->stateNum << "\n";
            if (visited.find(transition.second) == visited.end()) {
                stateQueue.push(transition.second); // 如果转移的目标状态未被访问，将其加入队列
                visited.insert(transition.second); // 标记为已访问
            }
        }
        std::cout << "\n"; // 打印空行以分隔不同状态的输出
    }
...
void LALR::printParsingTable() const {
    std::cout << "ACTION TABLE:" << std::endl;
    for (const auto& i : actionTable) {
        const auto &state = i.first;
        const auto &actions = i.second;
        std::cout << "State " << state << ":" << std::endl;
        for (const auto& j : actions) {
           const auto &symbol = j.first;
            const auto &action = j.second;
    ...
    for (const auto& i: gotoTable) {
        const auto& state = i.first;
        const auto& gotos = i.second;
        std::cout << "State " << state << ":" << std::endl;
        for (const auto& j : gotos) {
            const auto& symbol = j.first;
            const auto& nextState = j.second;
            std::cout << "\t" << symbol << " : " << nextState << std::endl;
        }
    }
}

...
void parser::preOrderTraversal(TreeNode* node) {
    if (node == NULL) {
        return;
    }
    // 访问当前节点
...
TreeNode* parser::statement(scaner& s)
{
    ...
    case PLUS : t = regex_stmt(s); break;
    default: 
...
TreeNode* parser::assign_stmt(scaner& s)
{
    ...
    if ((t != NULL) && (token == ID)) // 如果t不为空且当前词法单元是ID
        t->attr.name = copyString(tokenString,s); // 复制ID的字符串到赋值语句的属性中
    match(ID,s); // 匹配ID
    match(ASSIGN,s); // 匹配赋值符号ASSIGN
    if (t != NULL) t->child[0] = exp(s); // 如果t不为空，解析表达式作为赋值语句的值
    return t; // 返回赋值语句节点t
}
...
TreeNode* parser::read_stmt(scaner& s)
{
    ...
    if ((t != nullptr) && (token== ID)) {
        t->attr.name = copyString(tokenString,s);
